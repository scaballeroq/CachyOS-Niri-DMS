...
## 6. Niri and Dank Material Shell Configuration (`niri_dms.sh`)

Provides integration, shortcuts, and direct IPC with the **Niri** scrollable-tiling compositor and **DMS** shell:
...
- **Dank Material Shell IPC**:
  - `dms-reload`: Reloads or restarts DMS shell
  - `dms-launcher`: Opens the application launcher
  - `dms-control`: Opens the Control Center (Wi-Fi, Bluetooth, Audio, profiles)
  - `dms-settings`: Opens Dank Material Shell settings
  - `dms-lock`: Locks the user session
  - `dms-set-wallpaper <path>`: Applies wallpaper via IPC
